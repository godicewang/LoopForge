...
- **极简交互。** 原生 macOS UI 将复杂的 Agent Loop 控制完全自动化；用户
  只需输入结果需求、调整少量直观参数，即可处理高度复杂的通用任务。
- **超级复杂场景的长任务能力。** 从一句话开始，在用户确认的权限和工作时长
  内，LoopForge 可以独立连续运行超过 20 小时，全程无需反复交互，面向
  企业级产品和高完成度交付目标持续推进。
- **任务结果高度可视化。** 每个完成任务都能直接打开 HTML 交付网页，通过
  结果、改动、真实截图、验证、限制和下一步清晰汇报完成情况，解决 vibe
  coding 中用户逐渐失去项目掌控感的问题。
- **全场景通用。** 三种 Agent 后端均可直接使用：已有 Codex/ChatGPT 账号、
  多种第三方 API，以及下载开源模型进行本地部署；API 同时兼容 Responses
  与 Chat Completions 协议。
- **完全开源。** LoopForge 应用全部源码采用 Apache 2.0 许可证，包括
  Loop 编排、恢复、审计、Graph、Watcher 和 HTML 交付报告实现。
